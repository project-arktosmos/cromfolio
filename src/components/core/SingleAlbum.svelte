<script lang="ts">
	...
	function formatAlbumType(albumType: string): string {
		...
		return typeLabels[albumType] || albumType;
	}
	...
	function createBackCoverTexture(): THREE.CanvasTexture {
		...
		// Album type badge at top
		const albumTypeLabel = formatAlbumType(album.albumType);
		ctx.fillStyle = '#c0a060';
		ctx.font = 'bold 28px Arial';
		ctx.textAlign = 'center';
		ctx.textBaseline = 'top';
		ctx.fillText(albumTypeLabel.toUpperCase(), 256, 50);

		// Decorative line under album type
