<script lang="ts">
	...
	let searchResults = $state<TvSearchResult[]>([]);
	let isSearching = $state(false);
	let searchError = $state<string | null>(null);
	let selectedResult = $state<TvSearchResult | null>(null);
	...
	// Search TV series using Rust backend
	async function search() {
		...
		try {
			const results = await searchTv(searchQuery.trim(), searchYear.trim() || undefined);
			...
			searchError = error instanceof Error ? error.message : 'Failed to search TV series';
	...
	async function selectResult(result: TvSearchResult) {
	...
	async function fetchAllImages(imdbId: string) {
		...
		const sources = ['tmdb', 'tvmaze', 'fanart', 'credits'];
		...
		try {
			...
			tvmazeImages = allImages
				.filter((img) => img.source === 'tvmaze')
				.map((img) => ({ ...img, selected: true }));
	...
	async function createAlbumAndCards() {
		...
		try {
			...
			if (exists) {
				toastService.warning('This TV series has already been added');
			...
			const album: Album = {
				...
				description: `TV Series (${selectedResult.year})`,
			...
			if (createdAlbum) {
				await createSource(createdAlbum.id, 'tv', 'imdb', selectedResult.imdbId);
				if (tmdbId) {
					await createSource(createdAlbum.id, 'tv', 'tmdb', String(tmdbId));
				}
			...
			{:else if isLoadingImages}
				<div class="flex-1 flex flex-col items-center justify-center gap-4">
					...
					<div class="text-xs space-y-1">
						{#each ['tmdb', 'tvmaze', 'fanart', 'credits'] as source}
				...
				<div class="tabs tabs-boxed tabs-xs mb-2">
					...
					>
						...
						{#if getImageCount('tvmaze') > 0}
							<span class="badge badge-xs ml-1"
								>{getSelectedCount('tvmaze')}/{getImageCount('tvmaze')}</span
							>
						{/if}
					</button>
					<button
					...
					<div class="space-y-4">
						<div class="flex gap-3">
							...
							<div class="flex-1">
								...
								<span class="badge badge-secondary badge-sm">TV Series</span>
								<a
						...
						{:else if isSourceAdded(selectedResult.imdbId)}
							<div class="alert alert-warning">
								<span class="text-sm">This TV series has already been added.</span>
							</div>
						{:else}
							<button
