...
import { searchTv, getContentDetails } from './api/omdb.js';
import { findByImdbId, getTvImages, getTvCredits } from './api/tmdb.js';
...
export async function extractTv(
	...
	const {
		...
		searchQuery,
		year,
		withCast = true, // TV shows typically include cast by default
		dryRun = false,
	...
	try {
		...
		if (!imdbId && searchQuery) {
			...
			const results = await searchTv(apiKeys.omdb, searchQuery, year);

			if (results.length === 0) {
				return {
					success: false,
					message: `No TV shows found for query: "${searchQuery}"`,
					stickersCreated: 0,
					tagsCreated: 0,
					imagesSkipped: 0
				};
		...
		const details = await getContentDetails(apiKeys.omdb, imdbId);
		console.log(`Title: ${details.title} (${details.year})`);
		if (details.totalSeasons) {
			console.log(`Seasons: ${details.totalSeasons}`);
		}
		...
		console.log(`TMDB ID: ${tmdbResult.tmdbId} (${tmdbResult.mediaType})`);

		// Step 5: Fetch images from TMDB
		console.log('Fetching images from TMDB...');
		const images = await getTvImages(apiKeys.tmdb, tmdbResult.tmdbId);
		console.log(`Found ${images.length} images`);

		// Step 6: Fetch cast (TV shows usually include cast)
		let cast: CharacterItem[] = [];
		if (withCast) {
			console.log('Fetching cast from TMDB...');
			cast = await getTvCredits(apiKeys.tmdb, tmdbResult.tmdbId);
		...
		await db.createProvider(source.id, 'tv', 'imdb', imdbId);
		...
		return {
			success: false,
			message: `Failed to extract TV show`,
			stickersCreated: 0,
			tagsCreated: 0,
			imagesSkipped: 0,
			error: errorMessage
...
function buildDescription(details: ContentDetails): string {
	const parts: string[] = [`TV Series (${details.year})`];

	if (details.totalSeasons) {
		parts.push(`${details.totalSeasons} season${parseInt(details.totalSeasons) > 1 ? 's' : ''}`);
	}
