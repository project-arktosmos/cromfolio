...
const adminRoutesPath = join(__dirname, '../src/routes/admin');
const groupsConfigPath = join(__dirname, '../src/data/admin-menu-groups.json');
const outputPath = join(__dirname, '../src/data/admin-menu.json');
...
/**
 * Load group configuration from admin-menu-groups.json
...
/**
 * Scan the admin routes directory and generate menu items
 */
function generateMenuItems() {
	...
	try {
		const entries = readdirSync(adminRoutesPath, { withFileTypes: true });

		for (const entry of entries) {
			// Only process directories (subpages)
			if (entry.isDirectory()) {
				const folderName = entry.name;
				const folderPath = join(adminRoutesPath, folderName);
		...
		console.warn('Admin routes directory not found, creating empty menu');
	}
...
try {
	...
	console.log(`✓ Generated admin-menu.json with ${items.length} items`);
} catch (error) {
