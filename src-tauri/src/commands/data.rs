...
/// Read an award event YAML file
/// In dev mode, reads from project root's ref directory
/// In production, reads from bundled resources
#[command]
pub fn read_award_file(app: AppHandle, event_id: String) -> Result<String, String> {
    let filename = format!("{}.yml", event_id);

    // Try multiple paths
    let paths_to_try: Vec<std::path::PathBuf> = vec![
        // Dev mode: project root (go up from src-tauri)
        std::env::current_dir()
            .unwrap_or_default()
            .join("..")
            .join("ref")
    ...
    for path in &paths_to_try {
        log::info!("Trying to read award file from: {:?}", path);
        if path.exists() {
            match std::fs::read_to_string(path) {
                Ok(content) => {
                    log::info!("Successfully read award file from: {:?}", path);
                    return Ok(content);
                }
                Err(e) => {
    ...
    Err(format!("Could not find award file for event: {}. Tried paths: {:?}", event_id, paths_to_try))
}
